(function ($) {
    ...
    $.fn.IconPicker = function (parameter) {
        ...
        if (parameter != undefined) {
            ...
            if (parameter.currSelectionName != undefined) __currSelectionName__ = parameter.currSelectionName;
            ...
            if (baseDiv.prop('tagName') == 'DIV') {
                ...
                imgObjs.each(function () {
                    ...
                    if (thisUrl == __currSelectionUrl__) {
                        ...
                        if (__hasNullIcon__)
                            __currSelection__ = $(this).index() + 1;
                        else
                ...
                __containerDiv__.find('div').click(function () {
                    ...
                    __currSelectionName__ = getSelectedName();
                    __containerDivDisp__.slideUp(100);

                    // trigger onDisplayChange event
                    __onDisplayIconChange__(__currSelection__, __currSelectionUrl__, __currSelectionName__);
                });
            }
        });
        ...
        function getSelectedName() {
            var retUrl = getSelectedUrl();
            if (retUrl != '') {
                retUrl = retUrl.split('/');
                retUrl = retUrl[retUrl.length - 1];
            } else {
                retUrl = 'bala.IconPicker.nullIcon.png';
            }
            return retUrl;
        };
