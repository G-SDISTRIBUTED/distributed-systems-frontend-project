(function( $ ){
	...
	$.fn.multiple_emails = function(options) {
		...
		{
			deleteIconHTML = '<a style="color: #676a6c;size: 10px" href="#" class="multiple_emails-close" ><i style="size: 12px" class="glyphicon glyphicon-remove"></i></a>';
            EditIconHTML = '<a style="color: #676a6c ;size: 10px" href="#" class="multiple_emails-close" ><i style="size: 12px" class="glyphicon glyphicon-pencil"></i></a>';
		}
		...
		return this.each(function() {
			...
			if ($(this).val() != '' && IsJsonString($(this).val())) {
				$.each(jQuery.parseJSON($(this).val()), function( index, val ) {
                    ...
                    var emailList="<li class='multiple_emails-email'><span class='email_name' data-email='" + val.toLowerCase() + "'>" + val + "</span></li>";

					$list.append($(emailList)

                        .prepend($(EditIconHTML)
                            .click(function(e) {  $('#email_tool').val(val.toLowerCase());   $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
                        )
			...
			var StringInput='<input id="email_tool" type="text" class="multiple_emails-input text-left form-control" />';


			if(!settings.enabled){
            StringInput='<input disabled type="hidden" class="multiple_emails-input text-left" />';
			}



			var $input = $(StringInput).on('keyup', function(e) { // input
			...
			function display_email(t, dupEmailCheck) {
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
						...
						var emailStr=arr[i].toLowerCase();

						$list.append($('<li class="multiple_emails-email"><span class="email_name" data-email="' + arr[i].toLowerCase() + '">' + arr[i] + '</span></li>')

                            .prepend($(EditIconHTML)
                                .click(function(e) {  $('#email_tool').val(emailStr);   $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
                            )
